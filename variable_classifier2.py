...
import python_ast_utils2 as python_ast_utils
...
class VariableClassifier(BaseEstimator):
    ...
    def predict(self, tree, verbose = False):
        ...
        while stk:
            ...
            if node._label == 'FunctionDef':
                ...
                # that is called
                if node._children[0]._label != 'Name' or  len(node._children[0]._children) > 0:
                    raise ValueError('This classifier can only handle function calls which directly call a function.')
    ...
    def fit(self, trees, verbose = False):
        ...
        if verbose:
            print('collected %d function names, %d class names, and %d variable names' % (len(self.fun_names_), len(self.class_names_), len(self.var_names_)))
