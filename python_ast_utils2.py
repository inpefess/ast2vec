...
def tree_to_ast(tree):
    ...
    if tree._label == 'Compare':
        ...
        i = 1
        while i < len(tree._children) and tree._children[i] in _cmp_ops:
            i += 1
...
def parse_asts(srcs, filter_uniques = False, ids = None, ignore_syntax_errors = False, verbose = False):
    ...
    for i in progbar(range(len(srcs))):
        try:
            ...
            as_lists.append(x.to_list_format())
            parser.parse(*as_lists[-1])
    ...
    if filter_uniques:
        ...
        for i in progbar(range(len(trees))):
            x = trees[i]
            nodes, adj = as_lists[i]
            is_unique = True
            for j, (nodes2, adj2) in enumerate(lists_filtered):
            ...
            if is_unique:
                ...
                lists_filtered.append((nodes, adj))
